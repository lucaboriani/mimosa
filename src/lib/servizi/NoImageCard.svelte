<div class="w-full sm:w-1/2 md:w-1/2 lg:w-1/3   flex flex-col  ">
    <div class="flex flex-col h-full  mx-4 my-4 mt-4 mb-4 ml-4 mr-4  { bgColor ? bgColor :'bg-gray-0'}">
        <div class="h-3/5  {titleBgColor ? titleBgColor : ' bg-white '}">
            <h3 class="text-xl sm:text-xl {titleColor ? titleColor : ' text-gray-500 '}  p-4 pt-12 ">
                {title}
            </h3>
        </div>
        <div class="h-2/5 flex flex-col">
            ...
            {/if}
            {#if price}
                <p class="text-md  text-gray-500 text-right mt-auto mb-2">
