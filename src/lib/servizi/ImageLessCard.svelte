<div class="w-full sm:w-1/2 md:w-1/2 {width ? width : ' lg:w-1/3' }   flex flex-col  ">
    <div class="flex flex-col h-full  mx-4 mt-2 mb-2 ml-4 mr-4  { bgColor ? bgColor :'bg-gray-0'} justify-between">
        <div class="h-full  {titleBgColor ? titleBgColor : ' bg-white '} pb-4">
            <h3 class="text-xl sm:text-xl {titleColor ? titleColor : ' text-gray-500 '}  p-4  ">
                {title}
            </h3>
            {#if paragraphs && paragraphs.length > 0}
                {#each paragraphs as p}
                    <p class="text-md text-gray-600 italic mt-auto ml-4 mb-2">
                        {@html p }
                    </p>
                {/each}
            {/if}
        ...
        <div class=" flex flex-col pt-2">
            ...
            {#if price}
                <p class="text-md  text-gray-500 text-right uppercase mt-auto ">
                    ...
                    {:else}
                        ...
                        {/if}    
                        {#if price}
                            <bold class="text-gray-600  text-md pr-2">{price}</bold>
                        {/if} 
